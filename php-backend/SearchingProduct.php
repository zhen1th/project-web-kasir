...
// Jika ada token di URL (saat pertama kali login)
    ...
    if (!$result || !$result['valid']) {
        ...
        header('Location: http://localhost:3000/login?redirect=searchingproduct.php');
        exit;
    }
...
if (isset($_GET['kode'])) {
    ...
    header("Location: searchingproduct.php?message=" . urlencode($success_message ?? $error_message));
    exit;
}

$No = 1;

// Query pencarian dengan kondisi user_id
if (isset($_POST['caridata'])) {
    $searchTerm = mysqli_real_escape_string($koneksiDatabase, $_POST['caridata']);
    $syntaxquery = "SELECT * FROM newproduct WHERE user_id = $user_id AND Nama_Produk LIKE '%$searchTerm%'";
} else {
    $syntaxquery = "SELECT * FROM newproduct WHERE user_id = $user_id";
}

$showdata = mysqli_query($koneksiDatabase, $syntaxquery);
?>
<!DOCTYPE html>
<html lang="id">
...
<body class="d-flex">
    ...
    <div class="flex-grow-1 d-flex flex-column">
        ...
        if (mysqli_num_rows($showdata) > 0) {
            ...
            while ($menampilkandata = mysqli_fetch_array($showdata)) {
                echo "<tr>
                        <td>{$No}</td>
            ...
            echo "<div class='alert alert-warning text-center' role='alert'>
                    <strong>Produk Tidak Ditemukan!</strong> Silakan coba pencarian lainnya.
                  </div>";
        }
        ?>
    </div>
